...
class ProductoFisico(Producto):
    ...
    def __str__(self):
        return f"{self.nombre} (Físico) - ${self.precio + self.costo_envio}  (Stock: {self.stock})"
...
class Carrito:
    ...
    def __str__(self):
        return f"Carrito con {len(self.productos)} productos"
...
nuevoUsuario = Cliente(nombre, email, random.randint(1,10000000))
print(f"Bienvenido {nuevoUsuario.nombre} ({nuevoUsuario.id_cliente})! \n")
carrito = Carrito()

print("Se Agregaran 3 Productos al Carrito\n")
Producto1 = Producto("Libro", 20, 5)
Producto2 = ProductoDigital("Ebook", 20)
Producto3 = ProductoFisico("Camisa", 20, 3, 5)
